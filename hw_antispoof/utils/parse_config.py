...
from hw_antispoof import text_encoder as text_encoder_module
from hw_antispoof.base.base_text_encoder import BaseTextEncoder
from hw_antispoof.logger import setup_logging
from hw_antispoof.text_encoder import CTCCharTextEncoder
from hw_antispoof.utils import read_json, write_json, ROOT_PATH
...
class ConfigParser:
    def __init__(self, config, resume=None, modification=None, run_id=None):
        ...
        :param run_id: Unique Identifier for training processes.
                       Used to save checkpoints and training log. Timestamp is being used hw_antispoof default
        """
        ...
        if run_id is None:  # use timestamp hw_antispoof default run-id
    ...
    def init_obj(obj_dict, default_module, *args, **kwargs):
        """
        Finds a function handle with the name given hw_antispoof 'type' in config, and returns the
    ...
    def get_default_configs(cls):
        config_path = ROOT_PATH / "hw_antispoof" / "config.json"
        with config_path.open() as f:
            return cls(json.load(f))

    @classmethod
    def get_test_configs(cls):
        config_path = ROOT_PATH / "hw_antispoof" / "tests" / "config.json"
        with config_path.open() as f:
            return cls(json.load(f))
