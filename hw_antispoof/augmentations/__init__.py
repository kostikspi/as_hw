...
import hw_antispoof.augmentations.spectrogram_augmentations
import hw_antispoof.augmentations.wave_augmentations
from hw_antispoof.augmentations.sequential import SequentialAugmentation
...
def from_configs(configs: ConfigParser):
    ...
    if "augmentations" in configs.config and "wave" in configs.config["augmentations"]:
        for aug_dict in configs.config["augmentations"]["wave"]:
            wave_augs.append(
                configs.init_obj(aug_dict, hw_antispoof.augmentations.wave_augmentations)
            )
    ...
    if "augmentations" in configs.config and "spectrogram" in configs.config["augmentations"]:
        for aug_dict in configs.config["augmentations"]["spectrogram"]:
            spec_augs.append(
                configs.init_obj(aug_dict, hw_antispoof.augmentations.spectrogram_augmentations)
            )
